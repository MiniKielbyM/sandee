...
function FIRE_ACTION(x, y, i) {
  ...
  //flesh
  if (random() < 50) {
    const fleshLoc = borderingAdjacent(x, y, i, FLESH);
    if (fleshLoc !== -1) {
      gameImagedata32[fleshLoc] = FIRE;
      return;
    }
  }
...
function URANIUM_ACTION(x, y, i){
  if(bordering(x, y, i, FIRE) !== -1){
    particles.addActiveParticle(NUKE_PARTICLE, x, y, i*50000);
    particles.addActiveParticle(NUKE_PARTICLE, x, y+20, i*50000);
    particles.addActiveParticle(NUKE_PARTICLE, x+20, y, i*50000);
    particles.addActiveParticle(NUKE_PARTICLE, x, y-20, i*50000);
    particles.addActiveParticle(NUKE_PARTICLE, x-20, y, i*50000);
    gameImagedata32[i] = BACKGROUND;
    return;
